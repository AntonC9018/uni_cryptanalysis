// https://raw.githubusercontent.com/dhuertas/DES/master/des.c
module des;
import std.typecons;
private:
/*
...
immutable ubyte[] iteration_shift = [
...
ulong crypt(ulong input, ulong key, Flag!"encrypt" encrypt) @nogc pure nothrow
{
    ...
    foreach (i; 0..16)
    {
        ...
        foreach (j; 0..8)
        {
            ...
            row = cast(ubyte)((s_input & (0x0000840000000000 >> 6 * j)) >> 42 - 6 * j);
            row = (row >> 4) | row & 0x01;

            column = cast(ubyte)((s_input & (0x0000780000000000 >> 6 * j)) >> 43 - 6 * j);
...
unittest
{
    ...
    Result[8] expectedResults = [
        { 0x8da744e0c94e5e17, 0x0cdb25e3ba3c6d79 },
        ...
        { 0x95ec2578c2c433f0, 0x1b1a2ddb4c642438 },
    ];
    ulong input = 0x9474B8E8C73BCA7D;
    ulong result = input;
    foreach (expectedResult; expectedResults)
    {
        result = crypt(result, result, Yes.encrypt);
        assert(result == expectedResult.encryption);
